...
for p in bookpages/*; do
	...
	autor=$(grep -E 'Autor:|Autora:|Autores:' $p|sed 's/<br/\n/g'|grep -E 'Autor:|Autora:|Autores:'|sed 's/.*<strong/<strong/g'|cut -d\> -f3|cut -d \< -f1|sed -e 's/^[ \t]*//'|sed 's/ //');
	epub=$(grep "wpdm-download-link btn" $p|grep -v "mobi\|-2/"|cut -d\' -f6);
	mobi=$(grep "wpdm-download-link btn" $p|grep    "mobi\|-2/"|cut -d\' -f6);
